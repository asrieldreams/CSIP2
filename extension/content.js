...
function initEmailScanner(provider) {
    ...
    function onDomChange() {
        // Quick check: did the sender change?
        const email = extractSenderEmail(provider);
        if (email && email !== lastCheckedEmail) {
            // New sender detected — fire right away
            clearTimeout(debounceTimer);
            tryCheck();
        } else {
            // General DOM churn — short debounce
            clearTimeout(debounceTimer);
...
function extractSenderEmail(provider) {
    try {
        if (provider === 'gmail') {
            // Try all known Gmail sender selectors in priority order

            // 1. Most reliable — span with email attribute
            const selectors = [
                '.gD[email]',
                'span.gD[email]',
                'span[email].gD',
            ...
            for (const sel of selectors) {
                ...
                if (el) {
                    ...
                    if (e && e.includes('@') && e.includes('.')) return e.toLowerCase().trim();
                }
            }

            // 2. Any element with email attribute containing @
            ...
            // 3. data-hovercard-id with email
            ...
            // 4. Extract from visible "from" text using regex
        ...
        console.log('[CSIP2] extractSenderEmail error:', e.message);
    }
    return null;
}
...
// Track last check time per email — allow re-showing after 10s
...
async function checkEmailSender(email, provider) {
    ...
    // Block duplicate checks within 10 seconds (prevents flicker on DOM thrash)
    if (now - lastAt < 5000) return;
    _lastEmailCheck[email] = now;

    // Remove any existing email banner before showing new one
    ...
    try {
        const res  = await fetch(
            `${API_BASE}/check?url=${encodeURIComponent(email)}`,
            { signal: AbortSignal.timeout(2500) }
        );
        const data = await res.json();

        console.log(`[CSIP2] Email sender result: ${data.status} for ${email}`);
        ...
        console.log('[CSIP2] Email check failed:', e.message);
    }
}


// ══════════════════════════════════════════════════════════════
//  EMAIL WARNING BANNER — injected into Gmail/Outlook view
// ══════════════════════════════════════════════════════════════
...
function showEmailWarning(email, data, severity, provider) {
    ...
    const accent    = isDanger ? '#f85149' : '#d29922';
    const bgColor   = isDanger ? 'rgba(248,81,73,0.08)'  : 'rgba(210,153,34,0.08)';
    const border    = isDanger ? 'rgba(248,81,73,0.35)'  : 'rgba(210,153,34,0.35)';
    const icon      = isDanger ? '🚨' : '⚠️';
    const label     = isDanger ? 'CONFIRMED SCAM SENDER' : 'SUSPECTED SCAM SENDER';
    ...
    banner.innerHTML = `
        ...
        <div style="display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:10px">
            <div style="display:flex;align-items:center;gap:9px">
                ...
                <div>
                    <div style="font-weight:800;color:${accent};font-size:13px;letter-spacing:0.02em">
                        ${label}
                    </div>
                    <div style="font-size:11px;color:#6e7681;margin-top:1px">
                        🛡️ CSIP2 ScamWatch Shield
                    </div>
                </div>
            </div>
            <button id="csip2-email-x"
                style="background:rgba(255,255,255,0.07);color:#8b949e;padding:3px 8px;font-size:11px;margin-left:8px;flex-shrink:0">
                ✕ Dismiss
            </button>
        </div>
        ...
        <div style="background:${bgColor};border:1px solid ${border};border-radius:7px;padding:9px 12px;margin-bottom:10px">
            <div style="font-size:10px;color:#6e7681;text-transform:uppercase;letter-spacing:0.07em;margin-bottom:3px">
                Flagged Sender
            </div>
            <div style="font-weight:700;color:${accent};font-family:monospace;font-size:13px;word-break:break-all">
                ${email}
            </div>
        </div>
        ...
        <div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:10px">
            <span style="background:${bgColor};border:1px solid ${border};border-radius:5px;
                         padding:3px 9px;font-size:11px;font-weight:600;color:${accent}">
                🏷️ ${scamType}
            </span>
            <span style="background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);
                         border-radius:5px;padding:3px 9px;font-size:11px;color:#8b949e">
                👥 ${count} community report${count !== 1 ? 's' : ''}
            </span>
            <span style="background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);
                         border-radius:5px;padding:3px 9px;font-size:11px;color:#8b949e">
                📧 Email sender
        ...
        <div style="font-size:12px;color:#8b949e;margin-bottom:12px;line-height:1.5">
            ${isDanger
                ? '⛔ <strong style="color:#e6edf3">Do NOT</strong> click any links, download attachments, or reply to this email. Report to ScamAlert.sg or call <strong style="color:#e6edf3">1800-722-6688</strong>.'
                : '⚠️ This sender has been flagged by the community. <strong style="color:#e6edf3">Proceed with caution</strong> and do not share personal information.'}
        </div>

        <!-- Action buttons -->
        <div style="display:flex;gap:8px;flex-wrap:wrap">
            <button id="csip2-email-report"
                style="background:#2f81f7;color:#fff;flex:1;min-width:120px">
                📢 Report Sender
            </button>
            <button id="csip2-email-ignore"
                style="background:rgba(255,255,255,0.06);color:#8b949e;flex:1;min-width:100px">
                Proceed Anyway
            </button>
        </div>
    ...
    document.getElementById('csip2-email-report').addEventListener('click', async () => {
        ...
        try {
            ...
            if (res.status === 201 || result.message) {
    ...
    // Auto-dismiss after 20 seconds for suspected (not for confirmed)
    if (!isDanger) {
        setTimeout(() => {
...
function showPageBanner(url, scamType, reportCount) {
    ...
    banner.innerHTML = `
        ...
        <div style="font-size:12px;color:#8b949e;margin-bottom:12px;line-height:1.5">
            Community flagged. <strong style="color:#e6edf3">Proceed with caution.</strong>
        </div>
    ...
    document.getElementById('csip2-report').addEventListener('click', async () => {
        ...
        try {
            const res = await fetch(`${API_BASE}/report`, {
                ...
                body: JSON.stringify({ indicator_type:'url', indicator:url,
                    scam_type: scamType||'Others',
